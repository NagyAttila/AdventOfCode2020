...
# input_ = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
expected = ('byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid') # 'cid' is excluded
solution = 0
passports = input_.split('\n\n')

for passport in passports :
    entries = re.split(' |\n', passport)
    entries = list(map(lambda e : e.split(':')[0], entries))
    ...
    if all( map( lambda e : e in entries , expected) ) :
        solution += 1

print("Part #1: ", solution)

# 4 valid and 4 invalid entries
# input_ = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
# hcl:#623a2f
#
...
def validate(k,v):
    r =  (k == 'byr' and len(v) == 4 and v.isdigit() and int(v) >= 1920 and int(v) <= 2002) or \
           (k == 'iyr' and len(v) == 4 and v.isdigit() and int(v) >= 2010 and int(v) <= 2020) or \
           (k == 'eyr' and len(v) == 4 and v.isdigit() and int(v) >= 2020 and int(v) <= 2030) or \
           (k == 'hgt' and ("cm" in v and int(re.findall("\d+", v)[0]) >= 150 and int(re.findall("\d+", v)[0]) <= 193)) or \
           (k == 'hgt' and ("in" in v and int(re.findall("\d+", v)[0]) >= 59  and int(re.findall("\d+", v)[0]) <= 76)) or \
           (k == 'hcl' and re.search('^#[0-9a-f]{6}$',v) is not None) or \
           (k == 'ecl' and v in ("amb", "blu", "brn", "gry", "grn", "hzl", "oth")) or \
           (k == 'pid' and re.search('^\d{9}$',v) is not None) or \
           (k == 'cid')
    print(k,v,r)
    return r
...
for passport in passports :
    entries = re.split(' |\n', passport)
    keys = list(map(lambda e : e.split(':')[0] , entries))
    values = list(map(lambda e : e.split(':')[1] , entries))
    print(keys, values)
    ...
    for e in expected :
        if not e in keys :
            ...
            i = keys.index(e)
            valid = valid and validate(keys[i], values[i])
