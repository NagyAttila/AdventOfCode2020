input_ = """posh teal bags contain 2 faded coral bags, 3 striped crimson bags, 1 faded red bag.
...
# input_ = """light red bags contain 1 bright white bag, 2 muted yellow bags.
...
# input_ = """a b bag contain 1 c d bag, 2 shiny gold bags, 3 e b bags."""

def fn(bags, full, bag):

    if "no other bags" in bag:
        return False

    found = ""
    for expected in bag.split('contain ')[1].split('.')[0].split(', '):

        b = expected.split(' ')
        if "shiny" == b[1] and "gold" == b[2]:
            return True

        for actual in bags.split('\n'):
            b_ = actual.split(' ')
            if b_[0] == b[1] and b_[1] == b[2] and fn(bags, full, actual):
                return True
    return False
...
# input_ = """shiny gold bags contain 3 dark blue bags, 1 dark violet bags.
# dark blue bags contain 2 dark violet bags.
# dark violet bags contain no other bags."""

# input_ = """shiny gold bags contain 2 dark red bags.
...
def fn(bags, bag):
    ...
    for expected in bag.split('contain ')[1].split('.')[0].split(', '):
        ...
        for actual in bags.split('\n'):
            b_ = actual.split(' ')
            if b_[0] == b[1] and b_[1] == b[2]:
                n_bags = fn(bags, actual)
                found += int(n_) * n_bags
                break
    return found + 1
...
solution = 0
for x in input_.split('\n'):
    if "shiny gold bags contain" in x:
        solution = fn(input_, x) - 1
        break
