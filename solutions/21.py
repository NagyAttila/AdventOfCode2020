...
# input_ = """mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
...
all_allergies = dict()
dictinonary = set()
dictinonary_counter = Counter()
for x in xs:

    words = re.findall("\w+\s",x.split('(')[0])
    words = [w[:-1] for w in words]
    dictinonary.update( words )
    dictinonary_counter += Counter(words)
    allergies = re.findall("\s\w+[,)]",x.split('(')[1])
    allergies = [a[1:-1] for a in allergies]

    for a in allergies:
        if a in all_allergies:
            all_allergies[a] += Counter(words)
        else:
            all_allergies[a] = Counter(words)

for k,v in all_allergies.items():
    i = 0
    ws = v.most_common()
    while not ws[i][0] in dictinonary:
        i += 1
    dictinonary.remove(ws[i][0])

solution_1 = 0
for w in dictinonary:
    solution_1 += dictinonary_counter[w]
...
for x in xs:
    ...
    for a in allergies:
        if a in all_allergies:
            all_allergies[a].append( set(words) )
        else:
            all_allergies[a] = [set(words)]

possibles = dict()
for k,v in all_allergies.items():
    possibles[k] = v[0]
    for s in v:
        possibles[k] = possibles[k] & s

changed = True
while any([len(v) > 1 for v in possibles.values()]) and changed == True:
    changed = False
    new_possibles = possibles.copy()
    # print(sum([len(v) > 1 for v in possibles.values()]))
    for k,v in possibles.items():
        if len(v) == 1:
            new_possibles.update({key:val-v for key,val in new_possibles.items() if key != k})

            # Has anything changed ?
            for key,val in new_possibles.items():
                if val != possibles[key]:
                    changed = True

    possibles = new_possibles.copy()

solution_2 = [v.pop() for k,v in sorted(possibles.items(), reverse=False)]
