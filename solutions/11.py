input_ = """LLLLLLLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLLL.LLLL.L.LLLLLLL.LLLLLLL.LLLLLL.LL.LLLL.LLLLLLLLLLLLLLL
...
xs = list(map(lambda x : x, input_.split('\n')))
H = len(input_.split('\n'))
W = len(input_.split('\n')[0])

def isValid(xs,h,w,c):
    def isValid_(x, m):
        return x>-1 and x<m

    global H, W
    return isValid_(h,H) and isValid_(w,W) and xs[h][w] in c

def countNeighbours(xs, h, w, c):
    count = 0
    if isValid(xs,h-1,w,c)   : count += 1
    if isValid(xs,h-1,w-1,c) : count += 1
    if isValid(xs,h,w-1,c)   : count += 1
    if isValid(xs,h+1,w,c)   : count += 1
    if isValid(xs,h+1,w+1,c) : count += 1
    if isValid(xs,h,w+1,c)   : count += 1
    if isValid(xs,h-1,w+1,c) : count += 1
...
def isAnyNeighbour(xs, h, w, c):
    return \
        isValid(xs,h-1,w,c)   or \
        isValid(xs,h-1,w-1,c) or \
        isValid(xs,h,w-1,c)   or \
        isValid(xs,h+1,w,c)   or \
        isValid(xs,h+1,w+1,c) or \
        isValid(xs,h,w+1,c)   or \
        isValid(xs,h-1,w+1,c) or \
        isValid(xs,h+1,w-1,c)
...
while change:
    ...
    for h in range(H):
        for w in range(W):
            if xs[h][w] == 'L' and not isAnyNeighbour(xs, h, w, '#'):
                new_xs[h] = new_xs[h][:w] + "#" + new_xs[h][w+1:]
                change = True
            elif xs[h][w] == '#' and countNeighbours(xs, h, w, '#') > 3:
                new_xs[h] = new_xs[h][:w] + "L" + new_xs[h][w+1:]
                change = True
    xs = new_xs

solution = sum(map(lambda x : x.count('#'), xs))
...
def untilFirst(xs, h, w, oh, ow, c):
    h += oh
    w += ow
    while isValid(xs, h, w, '.'+c):
        if isValid(xs, h, w, c):
            return True
        h += oh
        w += ow
...
def countNeighbours(xs, h, w, c):
    count = 0
    if untilFirst(xs,h,w, -1,  0, c) : count += 1
    if untilFirst(xs,h,w, -1, -1, c) : count += 1
    if untilFirst(xs,h,w,  0, -1, c) : count += 1
    if untilFirst(xs,h,w, +1,  0, c) : count += 1
    if untilFirst(xs,h,w, +1, +1, c) : count += 1
    if untilFirst(xs,h,w,  0, +1, c) : count += 1
    if untilFirst(xs,h,w, -1, +1, c) : count += 1
...
def isAnyNeighbour(xs, h, w, c):
    return \
        untilFirst(xs,h,w, -1,  0, c) or \
        untilFirst(xs,h,w, -1, -1, c) or \
        untilFirst(xs,h,w,  0, -1, c) or \
        untilFirst(xs,h,w, +1,  0, c) or \
        untilFirst(xs,h,w, +1, +1, c) or \
        untilFirst(xs,h,w,  0, +1, c) or \
        untilFirst(xs,h,w, -1, +1, c) or \
        untilFirst(xs,h,w, +1, -1, c)
    ...
    for h in range(H):
        for w in range(W):
            if xs[h][w] == 'L' and not isAnyNeighbour(xs, h, w, '#'):
                new_xs[h] = new_xs[h][:w] + "#" + new_xs[h][w+1:]
                change = True
            elif xs[h][w] == '#' and countNeighbours(xs, h, w, '#') > 4:
                new_xs[h] = new_xs[h][:w] + "L" + new_xs[h][w+1:]
                change = True
    xs = new_xs
